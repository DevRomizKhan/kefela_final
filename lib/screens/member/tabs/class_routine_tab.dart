      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Classes List
            Column(
              children: routines.map((routine) => _RoutineItem(
                routine: routine,
                isToday: isToday,
                currentDay: currentDay,
              )).toList(),
...
class _RoutineItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final classStatus = _getClassStatus();
    final isClassNow = classStatus == 'Now';
      ...
      child: Row(
        children: [
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            decoration: BoxDecoration(
              color: _getStatusColor(classStatus),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Text(
              classStatus,
              style: TextStyle(
                color: _getStatusTextColor(classStatus),
                fontSize: 10,
                fontWeight: FontWeight.bold,
              ),
            ),
  ...
  String _getClassStatus() {
    ...
    // Parse times
    final startTime = _parseTime(routine['startTime']);
    final endTime = _parseTime(routine['endTime']);
    final currentTime = TimeOfDay.fromDateTime(now);

    // Check if it's the same day and class is currently happening
    if (routineDay == currentDayName && _isTimeBetween(currentTime, startTime, endTime)) {
      return 'Now';
    }

    // Check if it's today but class hasn't started yet
    if (routineDay == currentDayName && _isTimeBefore(currentTime, startTime)) {
      return 'Today';
    }
  ...
  bool _isTimeBefore(TimeOfDay now, TimeOfDay target) {
    final nowInMinutes = now.hour * 60 + now.minute;
    final targetInMinutes = target.hour * 60 + target.minute;
    return nowInMinutes < targetInMinutes;
  }
}